        ...
        <div class="bg-white border-2 border-gray-300 shadow overflow-hidden sm:rounded-lg">
            <div class="container px-5 py-5 mx-auto">
                <div class="text-center mb-5 justify-center">
                    <h1 class=" sm:text-3xl text-2xl font-medium text-center title-font text-gray-900 mb-4">{{ __('Quiz Result') }}</h1>
                    <p class="text-md mt-10"> Dear <span class="font-extrabold text-blue-600 mr-2"> {{Auth::user()->name.'!'}} </span> {{ __('You have secured') }} <a class="bg-green-300 px-2 mx-2 hover:green-400 rounded-lg underline" href="{{route('userQuizDetails',$quizid) }}">{{ __('Show quiz details') }}</a></p>
                ...
                <div class="flex flex-wrap lg:w-4/5 sm:mx-auto sm:mb-2 -mx-2">
                    <div class="p-2 sm:w-1/2 w-full">
                        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
                            ...
                            <span class="title-font font-medium mr-5 text-purple-700">{{ __('Correct Answers') }}</span><span class="title-font font-medium">{{$currectQuizAnswers}}</span>
                        </div>
                    </div>
                    <div class="p-2 sm:w-1/2 w-full">
                        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
                            ...
                            <span class="title-font font-medium mr-5 text-purple-700">{{ __('Total Questions') }}</span><span class="title-font font-medium">{{$totalQuizQuestions}}</span>
                        </div>
                    </div>
                    <div class="p-2 sm:w-1/2 w-full">
                        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
                            ...
                            <span class="title-font font-medium mr-5 text-purple-700">{{ __('Percentage Scored') }}</span><span class="title-font font-medium">{{$quizPecentage.'%'}}</span>
                        </div>
                    </div>
                    <div class="p-2 sm:w-1/2 w-full">
                        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
                            ...
                            <span class="title-font font-medium mr-5 text-purple-700">{{ __('Quiz Status') }}</span><span class="title-font font-medium">{{ $quizPecentage > 70 ? 'Pass' : 'Fail' }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-wrap -m-4">
                ...
                <div class="p-4 md:w-1/2 w-full">
                    <form wire:submit.prevent="startQuiz">
                        @csrf
                        <h2 class="text-gray-900 text-lg font-medium title-font mb-5">{{ __('Take a Quiz') }}</h2>
                        <div class="relative mx-full mb-4">
                            <select name="section" id="section_id" wire:model="sectionId" class="block w-full mt-1 rounded-md bg-gray-100 border-2 border-gray-500 focus:bg-white focus:ring-0">
                                @if($sections->isEmpty())
                                <option value="">{{ __('No Quiz Sections Available Yet') }}</option>
                                @else
                                <option value="">{{ __('Select a Quiz Section') }}</option>
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3 text-sm">
                                <label for="learningMode" class="font-medium text-gray-700">{{ __('Learning Mode?') }}</label>
                                <p class="text-gray-500">{{ __('If checked, this will enable explanation tab for each question.') }}</p>
                            </div>
                        </div>
                        ...
                        <button type="submit" class="block w-full text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">{{ __('Start Quiz') }}</button>
                    </form>
                </div>
            </div>
        </div>
